...
def main(page: ft.Page):
    class Button_move_window(ft.FilledButton):
        def __init__(self):
            ...
            self.view_make_bib=None
            def to_test(e):
                if self.view_make_bib is None:
                    self.text="ページ作成中..."
                    self.update()
                    ...
                    self.update()
                    self.view_make_bib=UI_make_bibfile.view_bib_maker()
                page.views.append(self.view_make_bib)
                page.go(self.view_make_bib.route)
                print(page.route)
                self.text="bibtex 出力ページ"
                self.style=None
